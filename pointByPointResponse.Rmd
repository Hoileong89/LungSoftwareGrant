...
* Where did the hardware discussion come from?  This is not a hardware proposal
...
* I think I can get PET data from LIDC.  However, the problem is that what software should we write (SUV?).
...
* Easy fix---we limit to UVa and Penn data as alpha testers
...
* ANTs has proven useful in the neuro domain.  Note that it is not just us
that think this.  Eric Hoffman also thinks this (cite paper).
...
* I think this should be combined with 6) General usage problems.
...
10.  Reduce scope
    * (Page 5, Approach:  weaknesses) "nothing is said about vessel segmentation."

11.  Nodule detection
    * (Page 5, Approach:  weaknesses) "This again is an extremely complex problem"
...
* There are no publicly available tools:
...
* Respectfully disagree.  A separate group, Eric Hoffman has been designing lung software
for decades and he is aware of the need.
...
* We can use Eduardos data.  However, there is no clinical outcome or result.
