...
which interface specific pulmonary algorithms (e.g., nodule detection) with clinical
...
\textcolor{red}{(most of which are not available to the public)}.\footnote{\textcolor{red}{Several competitions have been held in recent years focused on the processing and
  ...
  tasks specific to those databases.}} Importantly, the goals of this
project would significantly support the National Library of Medicine's
(NLM) own open-source directives in that all software proposed in the
project would be developed using the established Insight ToolKit's
coding and testing standards with the specific objective that all
project code would be contributed for inclusion in future versions of
the Insight ToolKit (ITK) as we have done in the past.
\textcolor{red}{NLM's position on open-source stems from the} documented
benefits within the targeted communities for which the software is
developed and supported. In addition to the increase in research output
illustrated earlier, open-source permits \textcolor{red}{trainees} and
...
project is the inclusion of an extensive set of use cases from leading
pulmonary
...
from these studies will be provided to the public for any interested
researcher to apply to their own data. Any clinical findings of interest
will be published in traditional venues (e.g., Chest). In addition, we
will provide all the quantitative analysis scripts as a companion
release for the paper (e.g., see previous similar offerings from our
group {[}12, 13{]}).
...
\textcolor{red}{introduced "N4," which incorporates both an enhanced fitting
...
\textbf{Joint label fusion for multi-atlas segmentation.} Joint label
...
joint label fusion (JLF) algorithm of {[}46, 47{]} from our group is
...
{[}49{]}, {[}13{]}, {[}49{]}. Importantly, we have successfully extended
\textcolor{red}{JLF} to the challenging problem of applying prior-based
information to lung and lobe segmentation {[}50{]}.\_
...
\caption{ANTs core processing tools for multi-modal lung imaging
studies. Using ANTs processing tools, our team has developed several
...
publicly available tools translates directly into a paucity of baseline
...
incorporate the data from the proposed multi-atlas CT library. Regional
quantification of pathologies indicated by ground glass opacity can be
quantified from intensity-based Bayesian segmentation with a Markov
random field spatial priors (described below) following atlas-based lung
segmentation.
...
In this evaluation, ``raters'' refers to the segmentation from our
automated approach and the manual tracings of three trained individuals.
In terms of combined specificity and sensitivity, our automated
algorithm demonstrated superior performance with the added benefit of
being reproducible and less time-consuming.
...
latter are important for subjects with a bronchitic diseases.
...
recent work has demonstrated {[}Lovinfosse:2016aa{]} these indices can
...
functionality with complementary aspects of our previous work {[}80{]}
...
lobe, airway, and vessel segmentations whereas the 1H MRI atlas library
will include left/right lung segmentations and lobar estimations
{[}50{]}. Along with the annotated data we will provide the scripts and
documentation to allow \textcolor{red}{independent} reproduction of the
ITK-Lung results. \textcolor{red}{Finally, optimal} group templates
{[}22{]} for the two atlas libraries
...
visualization {[}85{]}, \textcolor{red}{the development of which} is led
...
contours {[}86{]} and manual delineation functionality, put together
...
month in the last year), and our 2006 paper on ITK-SNAP {[}85{]} has
...
datasets, image registration and texture based feature extraction {[}95,
96{]} may allow earlier detection of BOS in the preclinical phase,
...
\textbf{Novel PET/CT biomarkers for early prediction of treatment
response and survival in patients with non small cell lung cancer
(NSCLC).} PET and CT have traditionally played important roles in
diagnosis, staging, and predicting treatment response in NSCLC
{[}97--99{]}. Although many studies have demonstrated diagnostic and
prognostic value of tumor metabolic burden and morphological information
derived from PET/CT imaging measures {[}99, 100{]}, current ability to
predict treatment response and survival following radiation therapy in
NSCLC is far from satisfactory {[}101{]}. The poor performance of
...
available in other lesions/regions {[}102{]}; second, many
...
subtleties of tumor characteristics and tumor heterogeneity {[}103,
104{]}; and finally, group-level statistical analysis strategies are
typically adopted to evaluate imaging markers, which may not be
effective for personalized prediction {[}105{]}.
...
at evaluation sites toward the end of Year 2 (with quarterly software
updates thereafter), and testing will run through Year 4. Documentation
and dissemination efforts will take place throughout the course of the
project.
...
95. Barbosa, E. M., Jr, Song, G., Tustison, N., Kreider, M., Gee, J. C.,
...
96. Song, G., Mortani Barbosa, E., Jr, Tustison, N., Gefter, W. B.,
...
97. Lardinois, D., Weder, W., Hany, T. F., Kamel, E. M., Korom, S.,
...
98. Lee, H. Y., Lee, H. J., Kim, Y. T., Kang, C. H., Jang, B. G., Chung,
...
99. Horne, Z. D., Clump, D. A., Vargo, J. A., Shah, S., Beriwal, S.,
...
100. Kim, K., Kim, S.-J., Kim, I.-J., Kim, Y. S., Pak, K., and Kim, H.
...
101. Berman, A. T., Ellenberg, S. S., and Simone, C. B., 2nd.
...
102. Ryu, J.-S. and Hyun, I. Y. ``\textbf{Prognostic Impact of
...
103. Chicklore, S., Goh, V., Siddique, M., Roy, A., Marsden, P. K., and
...
104. Depeursinge, A., Foncubierta-Rodriguez, A., Van De Ville, D., and
...
105. Henderson, R. and Keiding, N. ``\textbf{Individual Survival Time
