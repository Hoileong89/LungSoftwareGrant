...
contribution would be innovative as it would meet an obvious
...
which interface specific pulmonary CT algorithms (e.g., nodule detection) with clinical
...
\item
  ...
  each library \textcolor{red}{for use as
  normalized coordinate systems for more localized (i.e., voxelwise analyses)}.
...
of Medicine's own open-source directives in that all software proposed
...
of the Insight ToolKit (ITK) as we have done in the past. It should also
be noted that open-source software, in general, has documented benefits
within the targeted communities for which it is developed and supported.
In addition to the increase in research output illustrated earlier,
...
to the brain-specific algorithms provided in our ANTs toolkit {[}11{]},
our project would include \textcolor{red}{several} essential algorithms
for analyzing lung images from different modalities, including CT, PET,
3He, and 1H MRI. \emph{A large number of algorithms have been proposed
in various technical venues, \textcolor{red}{(mostly
...
pulmonary medical image analysis, many of which have been developed by
our group.} To facilitate the usage of these algorithms, we will provide
...
\textcolor{red}{with regression testing performed} using different image
acquisition protocols, equipment, etc. to ensure quality and robustness
of the \textcolor{red}{software and} processed data. Such use cases
involving other groups would highlight existing deficiencies in
framework functionality which would then be addressed. These real-world
use cases were solicited representing as broadly as possible the
requirements of the community as well as the multiple modality and
...
proposed functionality has been previously reported in various articles
from our group (cf Table 1). This includes both the reporting of
methodoligical novelty and subsequent usage for clinical studies of
small to large cohorts.
...
{[}22, 23{]}. First, an average template is estimated via a voxel-wise
...
template estimate closer to the group mean---see Figure 1 for a
...
the ANTs toolkit, with significant community adoption, for performing
neuroimaging research (e.g., {[}13, 24--28{]}).
\emph{\textcolor{red}{Based on our pulmonary research collaborations, we believe that this functionality
would be equally valuable and impactful to the pulmonary research field.}
{[}29{]}.}
...
protocol {[}44{]}. In {[}45{]} we introduced an improvement of N3,
...
\textcolor{red}{
It has since become the new standard in the field.}
...
joint label fusion (JLF) algorithm of {[}46, 47{]} from our group is one
of the top performing JLF algorithms
\textcolor{red}{and widely considered state-of-the-art}. JLF is capable
of predicting anatomical labels with accuracy that rivals expert
anatomists {[}48{]}. It has proven its effectiveness not only in cardiac
data {[}49{]}, the human brain {[}13{]}, and in multiple modality canine
MRI {[}49{]}, \emph{but has also been successfully extended to the
challenging problem of applying prior-based information to lung and lobe
segmentation {[}50{]}.}
...
described core tools, as well as several others, have been part of ANTs
and ITK development efforts for more than a decade. The deficiency of
...
\item
  functional lung segmentaiton {[}38{]}
...
\caption{ANTs based core processing tools for lung imaging studies.
Using ANTs core processing tools, our team has developed several
lung-specific extensions such as ventilation-based segmentation, lung
and lobe estimation, and multi-modality pulmonary template building.
...
core tool set described in Sub-Aim 1a which would permit large-scale
processing of multi-modal pulmonary image data. To illustrate the use of
the software, allow for processing of other public and private data
sets, and provide baseline data for algorithmic comparison, we plan to
release CT and 1H MRI annotated atlas libraries, corresponding
templates, and data-generating scripts as described below. The third
component will be significant extensions to the well-known ITK-SNAP
software for an enhanced user experience through a full featured
graphical user interface to support interactive parameter tuning and an
extensive suite of tools for evaluation and visualization of
user-processed results.
...
publicly available tools translates directly into a lack of baseline
performance standards with which researchers can compare their own
algorithms {[}60{]}. This project constitutes a specific and overdue
response to this major deficiency in the field.}
...
algorithms which can be used by the pulmonary research community.}
\textcolor{red}{
Toward this end, a select set of tools with a track record of good performance, spanning
the range of core functionality and designed to facilitate expansion, will serve the community
as a well-vetted quantitative resource and baseline for future algorithmic development.}
Table 2 comprises core functionality for multi-modal lung analysis that
...
preliminary work. Using ANTs core tools
\textcolor{red}{with lung-specific modifications}, we have produced
several algorithms for core pulmonary imaging tasks.
...
{[}61{]}, co-opting such technologies is complicated in MRI by issues
such as RF coil inhomogeneity, presence and resolution of structural
detail, and the absence of a physically-based intensity scaling.
...
segmentation have proven highly successful in neuroimaging {[}46, 52{]}
...
data from the proposed multi-atlas CT library. Regional quantification
of pathologies indicated by ground glass opacity can be quantified from
intensity-based Bayesian segmentation with a Markov random field spatial
priors (described below) following atlas-based lung segmentation.
...
approach (cf Figure 2). Specifically, to generate the lobe segmentation
...
thoroughly described in {[}50{]} where we showed that lobar overlap
...
providing spatial prior probability maps derived from image-specific CT
data features such as fissures, airways, and blood vessels for
data-driven, subject-specific lobe segmentation {[}62{]}.
...
of potential benefit for pulmonary functional analysis. In {[}38{]}, we
...
segmentation} \textcolor{red}{), as reported in} {[}38{]},
\textcolor{red}{the original pipeline performs MR
inhomogeneity correction
followed by Bayesian segmentation with a Markov Random Field prior.}
Without ground truth data for evaluation, we used a consensus labeling
approach {[}63{]} to simultaneously estimate the true segmentation from
given ``raters'' which included the segmentation from our automated
approach and the manual tracings of three trained individuals. In terms
of combined specificity and sensitivity, our automated algorithm
demonstrated superior performance with the added benefit of being
reproducible and less time-consuming. Future improvements to this
pipeline include adding an alternating bias-correction and the
segmentation process which iterates to a more optimal solution in
addition to the incorporation of an ANTs-based implementation of the
patch-based denoising protocol described in {[}51{]}.

\textbf{Multi-modality lung template construction.} Although the
template construction algorithm described in {[}22{]} is, as pointed out
...
will be extended and refined to include patch-based similarity metrics
suitable for minimizing the computational footprint of large
image-registration yet providing accurate normalizations {[}12{]} for
pulmonary data {[}21{]}.
...
evaluation of 15 airway segmentation algorithms {[}77{]} for the
Extraction of Airways from CT challenge held in 2009, the top 2
performers were the algorithms of {[}78{]} and {[}79{]} with the latter
...
{[}78{]} which employs a sharpening of the airway branching edges and
adaptive cuboid volumes to provide a region-growing airway segmentation
approach. We then plan to augment this functionality with complementary
aspects of our previous work {[}80{]} for removing leakage path
candidates.
...
the algorithms described in} {[}81{]}.
...
flow was first presented in} {[}82{]}
\textcolor{red}{and extended for use in assessing
pulmonary blood flow} {[}83,
84{]}\textcolor{red}{.  This deconvolution strategy was recently
implemented by our group for a separate lung imaging study but will be modified for this particular
...
\textcolor{red}{Most of the computational time in histology reconstruction
is spent during deformable registration
between pairs of lung images of approximately $512 \times 512 \times {\sim}500$ voxels.
Currently, in ANTs, with the parameters used
in our pipeline, the registration of a pair of lung images takes ${\sim}1-2$ hours on a single CPU core.}
In preliminary work, we have developed a CPU-based accelerated version
of the ANTs deformable registration algorithm that is approximately 15
times faster in typical problems than the version currently in use. This
means that on a typical 8-core CPU, the same registration of
\textcolor{red}{$512 \times 512 \times {\sim}500$ voxel images} can now
be done in under 20 minutes. This dramatic improvement was made
primarily though 1) use of more efficient metric gradient computation
algorithms, 2) use of single instruction multiple data (SIMD)
parallelization on Intel CPUs, and 3) extensive code profiling and
optimization. With the move to the GPU, we expect to speed up
registration by another factor of 4--5 times. Therefore, the vast
majority of ITK-Lung users will not need to have access to
high-performance computing clusters or other expensive hardware.
...
Sub-Aim 1c) will provide a user interface front-end to existing
\textcolor{red}{processing pipelines} running remotely. It will send
data to the external resource over the network, monitor and control
pipeline execution, and receive results when processing is complete.
This remote pipeline execution mode will additionally support other
highly computationally intensive tasks and other data processing tasks
that may be added in the future. The external resource may be either a
physical Linux cluster or a virtual Linux cluster hosted on the Amazon
Web Services (AWS) cloud. For AWS, we will use the MIT StarCluster
software (\url{http://star.mit.edu/cluster}) to launch a virtual cluster
directly from the ITK-Lung GUI on demand. We will take advantage of the
GPU enabled AWS instances, allowing the speed-ups described above to be
...
lung segmentations and lobar estimations {[}50{]}. Along with the
...
{[}22{]} for the two atlas libraries, respectively, will be included as
...
user-guided medical image segmentation and data visualization {[}85{]},
...
segmentation functionality based on active contours {[}86{]} and manual
...
paper on ITK-SNAP {[}85{]} has been cited over 1400 times (Google
...
tutorials, housed online on dedicated websites {[}11, 88{]}. A similar
...
human lungs with single breath hold {[}89--91{]}. This project plans to
...
children {[}92{]}. Furthermore, improvements in image processing and
...
{[}93{]} or hand counts of visual defects were used to estimate the
ventilation defect volume {[}94{]}. Development of more advanced
...
77. Lo, P., Ginneken, B. van, Reinhardt, J. M., Yavarna, T., Jong, P. A.
...
78. Feuerstein, M., Kitasaka, T., and Mori, K. ``\textbf{Adaptive Branch
...
79. Lee, J. and Reeves, A. P. ``\textbf{Segmentation of the Airway Tree
...
80. Song, G., Tustison, N., and Gee, J. C. ``\textbf{Airway Tree
...
81. Staring, M., Xiao, C., Shamonin, D. P., and Stoel, B. C.
...
82. Ostergaard, L., Weisskoff, R. M., Chesler, D. A., Gyldensted, C.,
...
83. Ohno, Y., Hatabu, H., Murase, K., Higashino, T., Kawamitsu, H.,
...
84. Hansch, A., Kohlmann, P., Hinneburg, U., Boettcher, J., Malich, A.,
...
85. Yushkevich, P. A., Piven, J., Hazlett, H. C., Smith, R. G., Ho, S.,
...
86. Caselles, V., Kimmel, R., and Sapiro, G. ``\textbf{Geodesic Active
Contours}'' \emph{Int J Comput Vision} 22, (1997): 61--79.

\hypertarget{ref-Zhu:1996aa}{}
87. Zhu, S. and Yuille, A. ``\textbf{Region Competition: Unifying
...
88. Available at \url{http://www.itksnap.org}

\hypertarget{ref-Dregely:2011aa}{}
89. Dregely, I., Mugler, J. P., 3rd, Ruset, I. C., Altes, T. A., Mata,
...
90. Mugler, J. P., 3rd, Altes, T. A., Ruset, I. C., Dregely, I. M.,
...
91. Qing, K., Mugler, J. P., 3rd, Altes, T. A., Jiang, Y., Mata, J. F.,
...
92. Altes, T. A., Mata, J., Lange, E. E. de, Brookeman, J. R., and
...
93. Woodhouse, N., Wild, J. M., Paley, M. N. J., Fichele, S., Said, Z.,
...
94. Altes, T. A., Powers, P. L., Knight-Scott, J., Rakes, G.,
