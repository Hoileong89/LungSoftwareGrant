...
research community. \textcolor{red}{Given the signifcant efforts
...
need through a first-of-its-kind software package for lung image analysis.}}
...
\textcolor{red}{These efforts would easily
...
\textcolor{red}{Such an original and innovative software package would potentially have an immediate and
significant impact.}} In fact, in a recent review of CT- and MRI-derived
...
\textcolor{red}{(many of which are not available to the public)}.\footnote{\textcolor{red}{Several competitions have been held in recent years dealing with the processing and
  analysis of lung image data (e.g., VOLCANO09–--nodule detection, EMPIRE10—--motion estimation,
  LOLA11—--lung and lobe segmentation, and VESSEL12---vasculature segmentation).  As far as we can tell, the vast majority of
...
of Medicine's own open-source directives in that all software would be
...
\emph{Many algorithms have been proposed in various technical venues,
\textcolor{red}{(mostly
as textual descriptions without accompanying software,)} but that which
...
to set the standard in the field \textcolor{red}{for brain} {[}34{]},
...
40--44{]}). \emph{Similarly, this functionality has also demonstrated
...
joint label fusion (JLF) algorithm of {[}58, 59{]} from our group is one
...
As part of this project we have developed similar scripts targeting lung analyses.}
...
community.}
\textcolor{red}{Although we will obviously not provide the entire panoply of lung-based
...
{[}86{]}, co-opting such technologies is complicated by MRI-specific
...
\(0.873 \pm 0.063\)). We will extend this framework to pulmonary CT in
...
of airway changes to chronic lung disease. Table 2 provides an overview
...
employed Hessian-based filters for deriving a "vesselness" function.  They evaluated
different filtering strategies using the test data provided by the organizers of the
VESSEL12 challenge organized in conjunction with the IEEE International Symposium on
Biomedical Imaging (ISBI 2012).  Their approach ranked consistently as one of the top
performers of the challenge.  Given that the first author is a regular
...
implemented by our group for a separate project but will be modified for this particular
proposal to automatically select the arterial input region and volume of interest.}
