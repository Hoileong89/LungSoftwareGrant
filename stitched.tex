...
\_\_ Atlas-based lung segmentation.\_\_ Identification of anatomical
...
\_\_ Airway and vessel segmentation.\_\_ In describing the quantitative
...
evaluation of 15 airway segmentation algorithms {[}97{]} for the
...
the top 2 performers were the algorithms of {[}98{]} and {[}99{]} with
...
implementation of {[}98{]} and then augment this functionality with some
of our previous work {[}100{]} for removing leakage path candidates.
...
{[}\textbf{???}{]} where a k-nearest neighbor classification employing
local image features is built from a large training data set. We plan to
...
ITK-SNAP for qualitative assessment of the results produced by the
proposed algorithms in Sub-Aim 1a.} Several features will be added to
ITK-SNAP to enhance its usability and adoption by those interested in
the lung-specific algorithms developed in the Sub-Aim 1a. In order to
assess image registration results, we plan to extend SNAP to allow for
loading of ITK-style transforms to better visualize the mapping between
two images in separate ITK-SNAP windows. Users will also be able to
generate the entire ITK transform library on the fly using manual
annotations and intensity information increasing interaction during the
registration process. \emph{We believe that the proposed enhancements to
...
housed online on dedicated websites {[}101, 102{]}. A similar
infrastructure will be developed for the software resources proposed in
Aim 1. \emph{Most crucially, we believe that our track record with
ITK-SNAP as well as ANTs demonstrates our team's commitment to producing
...
tissue and blood in human lungs with single breath hold {[}103--105{]}.
...
children {[}106{]}. Furthermore improvements in image processing and
signal intensity analysis can accurately measure lung volume
compartments {[}36{]}. Co-investigators \textbf{Gerry Teague} and
...
{[}107{]} or hand counts of visual defects were used to estimate the
ventilation defect volume {[}108{]}. Development of more advanced
...
98. Feuerstein, M., Kitasaka, T., and Mori, K. ``\textbf{Adaptive Branch
...
99. Lee, J. and Reeves, A. P. ``\textbf{Segmentation of the Airway Tree
from Chest CT Using Local Volume of Interest}'' \emph{Proc. of second
international workshop on pulmonary image analysis} (2009):

100. Song, G., Tustison, N., and Gee, J. C. ``\textbf{Airway Tree
Segmentation by Removing Paths of Leakage}'' \emph{Proc. 3rd int.
workshop pulmonary image analysis} (2010): 109--116.

101. Available at \url{http://picsl.upenn.edu/software/ants/}

102. Available at \url{http://www.itksnap.org}

103. Dregely, I., Mugler, J. P., 3rd, Ruset, I. C., Altes, T. A., Mata,
...
104. Mugler, J. P., 3rd, Altes, T. A., Ruset, I. C., Dregely, I. M.,
...
105. Qing, K., Mugler, J. P., 3rd, Altes, T. A., Jiang, Y., Mata, J. F.,
...
106. Altes, T. A., Mata, J., Lange, E. E. de, Brookeman, J. R., and
...
107. Woodhouse, N., Wild, J. M., Paley, M. N. J., Fichele, S., Said, Z.,
...
108. Altes, T. A., Powers, P. L., Knight-Scott, J., Rakes, G.,
