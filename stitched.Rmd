...
__3(a.1) The importance of pulmonary image analysis tools for research and clinical investigation.__
...
Statistical Parametric Mapping (SPM) package [@Ashburner:2012aa], and several others.  Performing a pubmed query for any one of
...
_\textcolor{red}{However, despite the obvious implications for the pulmonary imaging
...
significant impact.}_  In fact, in a recent
...
\textcolor{red}{(many of which are not available to the public)}.[^1]
It is important to note that the goals of this project would
...
[^1]: \textcolor{red}{Several competitions have been held in recent years dealing with the processing and
...
* __\textcolor{red}{lung segmentation}__: [@Tustison:2011ad;@Barbosa:2011aa;@Qing:2014aa;@Qing:2014ab;@Tustison:2015aa]
* __\textcolor{red}{lobe segmentation}__: [@Qing:2014ab;@Tustison:2015aa]
...
for image registration, segmentation, and other core medical image analysis functionality [@ANTsWebsite].
...
this project.  The following list comprises several core software tools for
...
continues to set the standard in the field
\textcolor{red}{for brain} [@Klein:2009aa], \textcolor{red}{lung} [@Murphy:2011aa], \textcolor{red}{
...
closer to the group mean---see Figure 2 for a cohort-specific multi-modality brain template
...
also been successfully extended to the challenging problem of applying prior-based information to lung and lobe
...
### 3(c.2) __Specific Aim 1:__   To develop ITK-Lung, a set of open-source software tools for CT, 1H, and 3He pulmonary computational analysis
...
interactive parameter tuning and an extensive suite of tools for evaluation and visualization
of user-processed results.
...
_A primary impetus for this project is that, through extension and continued development
of ANTs and ITK functionality, we can make a significant impact on the pulmonary imaging
research community in both basic science and clinical workflows by developing lung-specific
algorithms which are easy to use as we have done for the neuroimaging community._
...
The following list comprises core functionality for CT analysis that would be incorporated into
ITK-Lung in addition to further enhancements to the registration and segmentation capabilities
...
this project (cf Table 1).  Using ANTs core tools \textcolor{red}{with lung-specific modifications},
we have produced several algorithms for core pulmonary imaging tasks.
...
with Jaccard overlap measures for the left and right lungs being $0.966\pm0.018$ and
...
augmented by image-specific CT data features such as fissures, airways, and blood vessels
...
it is sufficiently general such that it can potentially be applied
...
project is that many of these measurements can also be directly applied to discriminative
...
__Airway segmentation.__  In describing the quantitative CT lung indices,
...
atlas libraries for both CT and 1H MRI acquisitions.  As indicated above, such atlases provide
extremely useful prior information for performing robust and accurate lung and lobe segmentations
within their respective modalities.  Additionally, the accompanying annotations provide an
...
subjects to represent a range of lung size and shape and will be annotated according to modality.  The CT atlas library will include lung, lobe,
...
Along with the annotated data we will provide the scripts and documentation to allow reproduction of
the ITK-Lung results.  Additionally, two group templates [@Avants:2010aa] for the two atlas
libraries, respectively, will be included as part of these open data sets.
...
not currently available to the general community. Based on these considerations, we propose
to base the GUI front-end to ITK-Lung on the ITK-SNAP multi-platform, open-source tool
for interactive user-guided medical image segmentation and data visualization [@Yushkevich:2006aa], and whose
development is led by project investigator Paul Yushkevich. ITK-SNAP provides an effective
...
be extended to support evaluation of registration quality, including a dashboard of performance
...
to encourage development of new clinical interventions.  HXe MRI is the
...
[@Tustison:2015aa] that will be available for both proton MRI and CT.
