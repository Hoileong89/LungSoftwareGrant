...
\textcolor{red}{for use as
normalized coordinate systems for more localized (i.e., voxelwise analyses)}.  Both
...
This functionality has proven to be a vital component of the ANTs toolkit, with significant
...
_\textcolor{red}{Based on our pulmonary research collaborations, we believe that this functionality
would be equally valuable and impactful to the pulmonary research field.} [@Tustison:2013ad]._
...
capabilities) and a modified optimization formulation.  \textcolor{red}{
...
[@Wang:2013aa;@Wang:2013ab] from our group is one of the top performing JLF algorithms
...
__Field-leading open-source implementations.__  The previously described core tools, as well as several others, have been part of ANTs and
...
* functional lung segmentaiton [@Tustison:2011aa] (https://github.com/ntustison/He3LongitudinalAnalysis), and
* lung and lobe segmentation [@Tustison:2015aa] (https://github.com/ntustison/LungAndLobeEstimationExample).
...
![ANTs based core processing tools for lung imaging studies.
...
algorithms which can be used by the pulmonary research community._
\textcolor{red}{
...
Table 2 comprises core functionality for multi-modal lung analysis that would
be incorporated into ITK-Lung in addition to further enhancements to the registration and segmentation capabilities
...
\textcolor{red}{), as reported in} [@Tustison:2011aa],
...
time-consuming.  Future improvements to this pipeline include
adding an alternating bias-correction and the segmentation process which iterates to a
more optimal solution in addition to the incorporation of an ANTs-based implementation
of the patch-based denoising protocol described in [@Manjon:2010aa].
...
computational footprint of large image-registration yet
providing accurate
...
to provide a region-growing airway segmentation approach.  We then plan to augment this
...
\textcolor{red}{$512 \times 512 \times {\sim}500$ voxel images} can now be
done in under 20 minutes. This dramatic improvement was made primarily though 1)
...
In this mode, the ITK-Lung GUI (see Sub-Aim 1c) will provide a user interface front-end to  existing
\textcolor{red}{processing pipelines} running remotely. It will send data to the external resource
...
intensive tasks and other data processing tasks that may be added in the future.
The external resource may be either a physical Linux cluster or a virtual Linux
cluster hosted on the Amazon Web Services (AWS) cloud. For AWS, we will use the MIT StarCluster
