...
efforts, CT and 1H MRI multi-atlas libraries ($n=30$ per library) per will be provided as open data complete
with the corresponding lung, airway, vessel, and lobe segmentations according to modality.
In addition, we will provide the corresponding optimal intensity/shape templates.  Both
sets of data will be provided with the corresponding scripts in order to reproduce the results.
...
various partner sites.  We will disseminate the results of the evaluation through open-source
...
It is important to note that the goals of this proposal would
...
### 3(c.3) __Specific Aim 2.__  Validate and disseminate the developed resources by leveraging use cases from a broad network of partner investigators representing the state-of-the-science in lung imaging research
...
__3(c.3.1) Novel imaging biomarkers for COPD.__
...
structure in Chronic Obstructive Pulmonary Disease (COPD) using HP MRI.
...
twice during the course of the five-year study, and the regional
...
*	to determine if specific patterns of disease progression are associated with genetic markers identified in the larger COPDGene study, and
*	to determine if disease progression is in part caused by excessive stretch in regions of the lung next to blocked-off areas unable to inflate normally.
