...
to leverage and enhance these advanced packages to make a similar impact in the pulmonary
...
consensus is that ``[the absence of] universally available image analysis software'' is a major hinderance
...
Similar to the brain-specific algorithms provided in our ANTs toolkit, our
 project would include the most essential algorithms for analyzing lung
...
would highlight existing deficiencies in algorithmic functionality in our project
...
data will be returned to the corresponding providers with detailed instructions
...
In addition to these collaborative efforts, our own analyses at Penn and UVa will also be included.  Any clinical findings
...
following list comprises core functionality for CT analysis that would be incorporated into
ITK-Lung in addition to further enhancements to registration and segmentation capabilities
...
which will incorporate the data from the proposed multi-atlas CT library .
...
To generate the lobe segmentation in a target 1H or CT lung image, we first generate a
binary whole lung mask using the whole lung atlas-based estimation.  We
then register a set of CT lung masks which have beeen expertly annotated (as part of the multi-atlas
...
of the project using the open-data multi-atlas CT library created as part of Sub-Aim 1b.
...
__3(c.2.2) Sub-Aim 1b will provide two annotated multi-atlas libraries, one for CT and another for 1H MRI.
...
subjects to represent a range of age and gender and will be annotated according to modality.  The CT atlas library will include lung, lobe,
...
HXe MRI is anticipated to detect pathologic changes present in COPD patients with
...
values of HXe MRI, we will utilize the recently proposed lobar estimation algorithm
...
COPD. Subjects will be mostly drawn from the well-characterized
...
manual registration for lung CT bronchial morphologic changes and
...
__3(c.3.6) Deep functional phenotyping of COPD.__  Our collaborator __Hans-Ulrich
...
__3(c.3.7) Advanced image processing pipelines for MR image-guided pulmonary
...
of in vivo imaging biomarkers openly across platforms and centers.  This
...
__3(c.3.8) Functional MR imaging of the lungs using hyperpolarized and inert gases.__
...
__3(c.3.9) Multimodality imaging studies of pulmonary diseases.__  Our collaborator,
...
as primary reference and instructional material._

__3(c.3.10) Sub-Aim 2a will disseminate the results of the project through open-source publication of the
