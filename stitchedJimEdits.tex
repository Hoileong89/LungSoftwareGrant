...
\long\def\jim#1{{\color{green}*** {\bf #1} ***}}
...
\fix{I think I like original specific aims writeup better?}

Historically, the research and clinical communities (and their overlap)
have significantly benefited from computational image analysis packages,
particularly those softwares which have been tailored for specific
application domains. Although several such established packages exist
for neuroimaging research (e.g., FSL, FreeSurfer, AFNI, SPM), \emph{no
such package exists for pulmonary imaging analysis. The primary goal of
this project is to develop a robust, open-source image analysis toolkit
and dissemination platform specifically targeted at the pulmonary
research community. \textcolor{red}{Given the significant efforts
to make lung imaging datasets publicly available (such as LIDC and RIDER), this
contribution would be \jim{extremely} innovative as it would meet an obvious
need through a first-of-its-kind software package for \jim{multi-modality} lung image analysis.}}
...
performance. \jim{Finally, many of these analysis methods are patented and/or
integrated into proprietary commercial software packages which severely
limits accessibility to researchers.}\jim{ERIC is consultant - tone this down?}

As a corrective alternative, this project brings together leading
expertise in lung imaging research at the
\textcolor{red}{University of Pennsylvania} and
\textcolor{red}{University of Virginia} to develop, evaluate and deploy
under community support an open-source software toolkit targeted for
pulmonary imaging research. As principal developers of the popular,
...
\jim{These efforts would easily
integrate with related NIH-sponsored projects such as the Chest Imaging Platform}\footnote{\url{http://chestimagingplatform.org}}
\jim{
which seeks to interface specific pulmonary CT algorithms (e.g., nodule detection) with the
ITK-based, open-source Slicer tool.}\footnote{\url{http:/www.slicer.org}}
\jim{Make this sentence more generic (instead of naming CIP specifically), echoing Critique 1?}
In addition to the software, we will provide scripts, documentation, and
tutorial materials consistent with open-science principles. Formally,
this project is defined by the following specific aims:

\begin{itemize}
\tightlist
\item
  \textbf{Specific Aim 1:} \textbf{Develop ITK-Lung, a set of
  open-source software tools for CT,
  \jim{PET},
  proton, and He-3 pulmonary
  ...
  \jim{xx to enable XXXX [spell out why atlases may be useful]}.
  In addition, we will generate
  optimal intensity/shape templates from each library
  \jim{xx to enable XXX [spell out why templates may be useful]}.
  Both sets of data
...
\item
  ...
  \textcolor{red}{proposed framework} within the real-world context of
  \textcolor{red}{actual research studies with institutional and
  other external collaborators with the former being the primary "beta testers."}
  \jim{alternative version: This aim will evaluate and refine the proposed framework within the real-world context of pulmonary research being carried out at Penn and UVa in addition to various partner sites serving as secondary beta testers.}
...
\textbf{3(a.1) The importance of
\jim{pulmonary [DELETE THIS?]}
 image analysis tools for
...
\jim{DELETE figure 1?  or make smaller as two-column type figure, with regular grant text on one side - this is a bit of pain to create but i can send you sample latex to do this.}
...
\emph{\textcolor{red}{However, despite the obvious implications for the pulmonary imaging
community}, no such analogous set of tools exist for multi-modal
pulmonary-specific research.
\textcolor{red}{Such an original
\jim{and innovative [DELETE?]}
 software package would potentially have an immediate and
significant impact.}}
\jim{In fact [replace with Indeed?]},
in a recent review of CT- and MRI-derived
...
\textcolor{red}{(
\jim{many [most?]}
 of which are not available to the public)}.\footnote{\textcolor{red}{Several competitions have been held in recent years
 \jim{dealing with [focused on?]}
  the processing and
  analysis of lung image data
  \jim{double dashes in text to follow are not being formatted correctly - i think we need to use 3 dashes?}
  (e.g., VOLCANO09–--nodule detection, EMPIRE10—--motion estimation,
  LOLA11—--lung and lobe segmentation, and VESSEL12---vasculature segmentation).
  \jim{As far as we can tell [To the best of our knowledge?]},
  ...
  tasks specific to those databases.}} It is important to note that the
goals of this project would significantly support the National Library
of Medicine's own open-source directives in that all software
\jim{proposed in the project}
would be
developed using the established Insight ToolKit's coding and testing
standards with the specific objective that all project code would be
contributed for inclusion in future versions of the Insight ToolKit
(ITK) as we have done in the past. It should also be noted that
open-source software, in general, has documented benefits within the
targeted communities for which it is developed and supported. In
addition to the increase in research output illustrated earlier,
open-source permits students and researchers to learn specific
...
Given the lack of open-source solutions for
\jim{multi-modality}
...
include \textcolor{red}{several} essential algorithms for analyzing lung
images from different modalities, including CT,
\jim{PET},
3He, and 1H MRI.
\emph{
\jim{Many [A large number of?]}
 algorithms have been proposed in various technical venues,
\textcolor{red}{(mostly
as textual descriptions without accompanying software,)} but
\jim{that which [awkward?]}
...
\jim{extremely daunting [prohibitive?]}.
However, limiting algorithmic implementation to those
\jim{with which we are familiar [delete?]}
and have provided good performance for our clinical collaborations will provide
\jim {an extremely beneficial package with significant implications for the community [rephrase this, include more specific language regarding benefits, like baseline platform along line of critique 1}.}
...
inclusion of extensive use cases from leading pulmonary
\textcolor{red}{imaging research scientists at our home institutions
as well as other external collabotors with regression testing performed}
\jim{i suggest we keep original sentence but add 'with regression testing performed using'}
using different image acquisition protocols, equipment, etc. to ensure
quality and robustness of the \textcolor{red}{software and} processed
data. Such use cases involving other groups would highlight existing
deficiencies in framework functionality which would then be addressed.
These real-world use cases were solicited representing as broadly as
possible the requirements of the community as well as the multiple
modality and algorithmic variations which commonly occur.

\textcolor{red}{
\jim{Our institutional collaborators will comprise the group of primary beta testers. [Our institutional collaborators will comprise the primary evaluation team.]}
Processed data will be returned to the corresponding providers with detailed instructions
on reproducing these results in their own labs.
\jim{Other external collaborators who will also
provide feedback based on their interaction with the platform. [We have also partnered with leading pulmonary research groups outside of our home institutions who are familiar with our work and who will provide feedback based on their interaction with beta releases of the platform.]}}
\textcolor{red}{Tutorial materials, data, and example scripts} drawn
from these experiences will be provided to the public for any interested
researcher to apply to their own data. Given the different image
acquisition sources, this strategy should also demonstrate the
robustness of our tools. Any clinical findings of interest will be
...
13{]}).
\jim{Such a comprehensive clinical research investigation [reword this?]}
 using these
...
\jim{I agree about replacing this with table (and cite only lung related references).  I think we want to rephrase this introductory paragraph to specifically address/highlight our experience and expertise with lung imaging (as opposed to brain imaging, which reviewers are getting hung up on to our detriment).}
\textcolor{red}{(NB:  replace this with table once all the refs are stabilized.)
A significant portion of the proposed platform spanning the range of
proposed functionality has been previously reported in various articles from our group.
This includes both the reporting of methodoligical novelty and subsequent
usage for clinical studies of small to large cohorts.
The following list provides a representative sampling of this work categorized by
proposed functionality:}

\begin{itemize}
\tightlist
\item
  \textbf{\textcolor{red}{spatial normalization}}: {[}13--20{]}
\item
  \textbf{\textcolor{red}{template generation}}: {[}21{]}
\item
  \textbf{\textcolor{red}{lung segmentation}}: {[}19, 22--25{]}
\item
  \textbf{\textcolor{red}{lobe segmentation}}: {[}24, 25{]}
\item
  \textbf{\textcolor{red}{airway segmentation}}: {[}26{]}
\item
  \textbf{\textcolor{red}{functional segmentation}}: {[}21, 27, 28{]}
\item
  \textbf{\textcolor{red}{feature indices}}: {[}22, 29, 30{]}
\end{itemize}

\textcolor{red}{Much of this innovation has been provided through the ANTs processing toolkit to
allow for continued use, potential future improvements, and reproducibility.}
...
\jim{core [basic]}
 medical image analysis functionality
...
{[}31--33{]}. Our most recent work, embodied in the ANTs open-source,
cross-platform toolkit for multiple modality image processing, continues
to set the standard in the field
\jim{switch up order here, and list lung first! - for lung, cardiac and brain imaging}
\textcolor{red}{for brain} {[}34{]},
\textcolor{red}{lung} {[}35{]}, \textcolor{red}{
and cardiac imaging} {[}36{]}. ANTs not only encodes the most advanced
results in registration research, notably the Symmetric Normalization
(SyN) algorithm for diffeomorphisms {[}37{]}, but also packages these
...
available, annotated
\jim{brain [delete?]}
 data sets and demonstrated statistically
...
original counterpart on pulmonary data {[}20{]}.}
...
{[}38, 39{]}. First, an average template is estimated via a voxel-wise
...
range 50--60.
\jim{This functionality has proven to be a vital component of
the ANTs toolkit
[with significant community adoption]
for performing neuroimaging research (e.g., {[}13,
40--44{]}).}
\jim{[rephrase this sentence to emphasize how this functionality would be equally valuable and impactful to the pulmonary field and this project would serve to translate/bring to bear this technology to/on the field]
 \emph{Similarly, this functionality has also demonstrated
significant importance in pulmonary studies {[}21{]}.}
}

\textbf{Bayesian segmentation with spatial and
\jim{Markov Random Field}
priors.} Early
...
{[}45{]}. Following this work, many researchers adopted statistical
methods for \(n\)-tissue anatomical brain segmentation. The
Expectation-Maximization (EM) framework is natural {[}46{]} given the
...
statistical distributions {[}47, 48{]} and the use of Markov Random
Field (MRF) modeling {[}49{]} for regularizing the classification
results {[}50{]}. Spatial prior probability maps of anatomical
structures of interest are also employed within this framework {[}51,
52{]}. Although this particular segmentation framework has significant
application in the neuroimaging domain, it is also relevant to other
domains including functional ventilation of the lung {[}27{]}.
\emph{However, despite the numerous developments which have been
proposed over the years within this area, there are
\jim{an extremely limited
number of  [only a few?]}
actual software implementations. This deficit inspired us to
create our own Bayesian segmentation framework {[}53{]} (denoted as
...
bias correction algorithm, N3 {[}56{]}, as part of their standard
protocol {[}57{]}. In {[}17{]} we introduced an improvement of N3,
...
\jim{add something like N4 is the new standard in the field xxxx}
...
joint label fusion (JLF) algorithm of {[}58, 59{]} from our group is
\jim{one
of the top performing JLF algorithms [widely recognized as the current state of the art]}.
JLF is capable of predicting
anatomical labels with accuracy that rivals expert anatomists {[}60{]}.
It has proven its effectiveness not only in cardiac data {[}61{]}, the
human brain {[}13{]}, and in multiple modality canine MRI {[}61{]},
...
\jim{add subtitle here: Field leading open-source implementations. ??}
...
\item
  ...
  normalization evaluation {[}34{]}.
\item
  SyN also competed in the Evaluation of Methods for Pulmonary Image
  REgistration 2010 (EMPIRE10) challenge {[}35{]}, where it was the top
...
\item
  The joint label fusion algorithm of {[}58, 63{]} (coupled with SyN)
  was top-ranked in the MICCAI 2012 challenge for labeled brain data
  {[}64{]} and in 2013 for labeled canine hind leg data {[}65{]}.
\item
...
segmentation.
\jim{is this figure referenced anywhere?}}
\end{figure}
...
\jim{As part of this project we have developed similar scripts targeting lung analyses. [rework this to better segue into next Specific Aims sections]}
...
\jim{edit figure title to 'ANTs based core processing tools for lung imaging studies'. is this figure referenced anywhere?}
\end{figure}

\subsubsection{\texorpdfstring{3(c.2) \textbf{Specific Aim 1:} To
develop ITK-Lung, a set of open-source software tools for CT,
\jim{PET},
1H, and
...
\jim{multi-prong offering [multi-faceted approach?]}
with ITK-Lung. The main
\jim{componenent [component]} will be the core tool set
...
algorithms.}
\jim{Analogous to the neuroimaging tasks described earlier,
...
available package. [Many important algorithmic categories implementing fundamental lung image analysis tasks do not currently exist in any comprehensive, publicly available package.]}
 This is
 \jim{in spite of [despite]}
  the fact that new algorithms for
...
{[}70{]} which covers computer-aided diagnosis of lung disease and lung
...
{[}71{]}. This project constitutes a specific and overdue response to
...
algorithms which are easy to use
\jim{as we have done for the neuroimaging
community [replace this with 'easy to use and xxxxx'].}
\jim{rework this sentence: \textcolor{red}{Although we will obviously not provide the entire panoply of lung-based
image processing algorithms that have been proposed over the years, a
set of tools with a track record of good performance spanning the range of core
functionality (similar to our ANTs neuroimaging
toolkit), with the potential for expansion, will be a definite benefit to the community.}
[alternative version: Toward this end, a select set of tools with a track record of good performance, spanning the range of core functionality and designed to facilitate expansion, will serve the community as xxxxx (use language from critique 1 here?)]}

\jim{i think the next 2 paragraphs need to be reworked.  a table of functionality/tasks, with specific (as opposed to 'e.g.') references covering functionality/tasks that will be implemented.  need to conceptualize and present this as multi-modality.}
The following list comprises core functionality for CT analysis that
would be incorporated into ITK-Lung in addition to further enhancements
to the registration and segmentation capabilities described in
preliminary work:

\begin{itemize}
\tightlist
\item
  whole lung differentiation from the chest wall (e.g., {[}72--75{]}),
\item
  bronchial structure extraction (e.g., {[}76, 77{]} and the many
  ...
  Analysis {[}78{]}),
\item
  vasculature segmentation (e.g., {[}79, 80{]}),
\item
  lobe and/or fissure detection (e.g., {[}81, 82{]}), and
\item
  feature extraction and classification (e.g., {[}83--85{]}).
\end{itemize}
...
tools \textcolor{red}{with lung-specific modifications}, we have
produced several algorithms for core pulmonary imaging tasks.
\textcolor{red}{We have even provided as open-source initial implementations and github
examples for many of them.} These imaging tasks include:

\textbf{Atlas-based lung segmentation.} Identification of anatomical
structure in
\jim{MRI [replace with more generic 'lung images']}
 is often a crucial preprocessing step for
quantification of morphological features or ventilation information from
functional images. Quantitative regional analysis
\jim{often [typically?]}
 requires the
...
{[}86{]}, co-opting such technologies is complicated
\jim{by MRI-specific [replace with 'in MRI by']}
...
the left and right lungs in 1H MRI {[}25{]}. Multi-atlas approaches to
segmentation have proven highly successful in neuroimaging {[}58, 63{]}
...
proposed method {[}25{]} demonstrated excellent performance with Jaccard
overlap measures for the left and right lungs being \(0.966\pm0.018\)
and \(0.970\pm0.016\), respectively.
\jim{Further work for this project
...
[need to elaborate further to address criticism about lack of details]}
...
approach (cf Figure 3). Specifically, to generate the lobe segmentation
in a target 1H or CT lung image, we register the same-modality atlas set
to the target image (given the general increased robustness of
intra-modality vs.~inter-modality image registration) using the B-spline
SyN registration approach described earlier {[}12{]}. Subsequently, we
warp the set of lobe label images to the target image using the
...
thoroughly described in {[}25{]} where we showed that lobar overlap
measures in 1H MRI were on par with state-of-the-art CT methods where
fissure information is actually visible (left upper:
\(0.882 \pm 0.059\), left lower: \(0.868 \pm 0.06\), right upper:
\(0.852 \pm 0.067\), right middle: \(0.657 \pm 0.130\), right lower:
\(0.873 \pm 0.063\)).
\jim{We will extend this framework to pulmonary CT in
providing spatial prior probability maps augmented by image-specific CT
data features such as fissures, airways, and blood vessels for
data-driven, subject-specific lobe segmentation {[}82{]}.
[need to elaborate further to address criticism about lack of details]}
...
of potential benefit for pulmonary functional analysis. In {[}27{]}, we
presented an automated algorithmic pipeline for ventilation-based
partitioning of the lungs in hyperpolarized 3He and 129Xe MRI.
\jim{add sentence here to overview pipeline?}
Without
ground truth data for evaluation, we used a consensus labeling approach
{[}87{]} to simultaneously estimate the true segmentation from given
...
\jim{Since the initial development, we
...
patch-based denoising protocol described in {[}62{]}.
[rephrase this as future/proposed work (drop 'since the initial development') - add details if possible.]}

\textbf{Multi-modality lung template construction.} Although the
template construction algorithm described in {[}38{]} is, as pointed out
...
will be
\jim{extended and refined
[need to elaborate further to address criticism about lack of details]}
to provide accurate normalizations {[}12{]}
for pulmonary data {[}20{]}.
...
volumetric tissue (e.g., {[}88, 89{]}), distribution of low attenuation
areas (LAA) (e.g., {[}90, 91{]}), cooccurrence and run-length matrix
features (e.g., {[}83, 92{]}), attenuation statistics (e.g., {[}93,
94{]}), deformation measures (e.g., {[}14, 15{]}), and stochastic
fractal dimension features (e.g., {[}83, 94{]}); and those that
characterize the airways (e.g., {[}95--97{]}). The former are important
...
\jim{Table 2 provides an overview
of these types of discriminative measurements, many of which can be used
for CT and 3He lung assessment. We have already implemented many of
these image features and have contributed the result of our work to the
Insight Toolkit (e.g., {[}98, 99{]}).
[rework this to include future work.  may need to delete table]}

\input{ct_table.tex}

\textbf{Airway segmentation.} In describing the quantitative CT lung
indices, it was pointed out that lung airway morphology has been
...
evaluation of 15 airway segmentation algorithms {[}100{]} for the
Extraction of Airways from CT challenge held in 2009, the top 2
performers were the algorithms of {[}101{]} and {[}102{]} with the
latter being one of the more conservative algorithms and the former
being more prone to false positives.
\jim{Our plan is to provide an
implementation of {[}101{]} and then augment this functionality with
some of our previous work {[}26{]} for removing leakage path candidates.
[rework to provide overview sentence of 101, and elaborate further to
address criticism about lack of details]}
...
the algorithms described in} {[}103{]}.
\textcolor{red}{Similar to much of the
research for enhancement of vessel-like morphology in different anatomies, the authors
employed Hessian-based filters for deriving a
\jim{"vesselness" [``vesselness'']}
function.
\jim{They evaluated
different filtering strategies using the test data provided by the organizers of the [replace with 'Their approach ranked consistently among the top performers in the]}
VESSEL12 challenge organized in conjunction with the IEEE International Symposium on
Biomedical Imaging (ISBI 2012).
\jim{Their approach ranked consistently as one of the top
performers of the challenge. [delete this]}
  Given that the first author is a regular
...
flow was first presented in [@Ostergaard:1996aa] and extended for use in assessing
pulmonary blood flow [@Ohno:2004aa;@Hansch:2012aa].  This deconvolution strategy was recently
implemented by our group for a separate
\jim{project [lung imaging study]}
but will be modified for this particular
\jim{proposal [project]}
 to automatically select the arterial input region and volume of interest.}

\textbf{\textcolor{red}{PET imaging.}}

\jim{GPU, HARDWARE and DATA THROUGHPUT requirements, etc.  please see new text below.  note that i say GPU acceleration is already in ANTs for affine registration, which i don't think is true, so please reword if necessary.  the current examples are drawn from histology registration and therefore needs to be updated - please give it a shot.}

\jim{Graphics processing unit acceleration.
In preliminary work, we have successfully used OpenGL shader language to perform image processing operations, such as computation of similarity metrics between images or extraction of features from images, on the graphics processing unit (GPU) at real-time speed. GPU requirements for ITK-Lung will be very modest and would be met by virtually any mid-range desktop computer sold today (e.g., a mid-range iMac).
\textcolor{red}{We have already integrated
affine registration with GPU acceleration into ANTs, and now propose to integrate a GPU-accelerated implementation
of deformable registration as well.} The three components of diffeomorphic deformable registration
in ANTs that account for over 95\% of computation time are: 1) computation of the gradient of the similarity metric
(e.g., mutual information, cross-correlation); 2) smoothing of the gradient using Gaussian kernels; and 3) image
and vector field interpolation. All three components are highly parallelizable, and we will write OpenGL shader
code for performing these operations on the GPU, analogous to the code currently used for metric computation
in GPU-based affine registration in ANTs.
}

\jim{
Computational power. With the proposed GPU acceleration of image registration, we expect that
even the most complex registration-dependent problems will be possible to solve on a desktop computer in
under XXXX.
\textcolor{red}{Most of the computational time in histology reconstruction is spent during deformable registration
between pairs of images of approximately 1K by 1K pixels (while raw images can be much larger, it is unusual
that deformations on the scale of less than 10 ?m are of interest). For a stack of 100 sections, with default
parameters, the algorithm requires around 1200 such registrations. Currently, in ANTs, with the parameters used
in our pipeline, the registration of a pair of 1000�1000 pixel images takes ?280 seconds on a single CPU core.}
In preliminary work, we have developed a CPU-based accelerated version of
the ANTs deformable registration algorithm that is $\sim$15 times faster in typical problems than the version currently
in use. This means that on a typical 8-core CPU, the same registration of
\textcolor{red}{1000�1000 pixel images} can now be
done in under 4 seconds. This dramatic improvement was made primarily though 1) use of more efficient metric
gradient computation algorithms, 2) use of single instruction multiple data (SIMD) parallelization on Intel CPUs,
and 3) extensive code profiling and optimization. With the move to the GPU, we expect to speed up registration
by another factor of 4--10 times, particularly because
\textcolor{red}{each registration problem is small enough to fit into GPU
texture memory,} reducing the bottlenecks from random accesses to main memory. Thus, we are confident that on
the typical low-cost GPU, deformable registration for \textcolor{red}{one pair of 1000�1000 sections can be performed in under
a second, and a complete stack alignment in 30 minutes or less.} Therefore, the vast majority of ITK-Lung users
...
In this mode, the ITK-Lung GUI (see Sub-Aim 1c) will provide a user interface front-end to  existing \textcolor{red}{processing pipelines} running remotely. It will send data to the external resource over the network, monitor and control pipeline execution, and receive results when processing is complete. This remote pipeline execution mode will additionally support other highly compute intensive tasks, such as running \textcolor{red}{XXXX}, and other data processing tasks that may be added in the future.
The external resource may be either a physical Linux cluster or a virtual Linux
cluster hosted on the Amazon Web Services (AWS) cloud. For AWS, we will use the MIT StarCluster software (\url{http://star.mit.edu/cluster}) to launch a virtual cluster directly from the ITK-Lung GUI on demand. We will take
advantage of the GPU enabled AWS instances, allowing the speed-ups described above to be combined with
...
AWS will incur negligible costs (\$0.01 per GB), particularly since data may be uploaded at  \textcolor{red}{reduced resolutions
...
lung segmentations and lobar estimations {[}25{]}. Along with the
...
{[}38{]} for the two atlas libraries, respectively, will be included as
...
user-guided medical image segmentation and data visualization {[}104{]},
...
segmentation functionality based on active contours {[}105{]} and manual
...
paper on ITK-SNAP {[}104{]} has been cited over 1400 times (Google
...
tutorials, housed online on dedicated websites {[}11, 107{]}. A similar
...
\textcolor{red}{Two groups are considered
below as ``users'' of the software:  institutional (i.e., beta testers) and external collaborators.
For the former, the developers of the proposed platform will provide one-on-one training
sessions to ensure proper data processing by the beta testers.  We will use these sessions
to develop online tutorials and processing examples for our external collaborators such that
they will be able to process their data in-house.  We will also provide a targeted discussion
forum to answer questions and address concerns for processing using ITK-Lung.}
...
human lungs with single breath hold {[}24, 108, 109{]}. This project
plans to establish connectivity between these advanced HXe MRI imaging
...
functionality in ITK-Lung {[}27{]}. In order to determine lobar values
of HXe MRI, this study will utilize the recently proposed lobar
estimation algorithm {[}25{]} that will be available for both proton MRI
and CT.
...
children {[}110{]}. Furthermore, improvements in image processing and
signal intensity analysis have made possible accurate measurements of
lung volume compartments {[}27{]}. Co-investigators \textbf{Gerry
...
{[}111{]} or hand counts of visual defects were used to estimate the
ventilation defect volume {[}112{]}. Development of more advanced
...
for these data include denoising techniques {[}62{]} implemented and
made available in ANTs. Lobe estimation will be possible by refining the
techniques originally described in {[}25{]}.
...
14. Gee, J., Sundaram, T., Hasegawa, I., Uematsu, H., and Hatabu, H.
...
15. Sundaram, T. A. and Gee, J. C. ``\textbf{Towards a Model of Lung
...
\hypertarget{ref-Cook:2007aa}{}
16. Cook, T. S., Tustison, N., Biederer, J., Tetzlaff, R., and Gee, J.
``\textbf{How Do Registration Parameters Affect Quantitation of Lung
Kinematics?}'' \emph{Med Image Comput Comput Assist Interv} 10, no. Pt 1
(2007): 817--24.

\hypertarget{ref-Tustison:2010aa}{}
17. Tustison, N. J., Awate, S. P., Cai, J., Altes, T. A., Miller, G. W.,
...
\hypertarget{ref-Tustison:2011ab}{}
18. Tustison, N. J., Cook, T. S., Song, G., and Gee, J. C.
``\textbf{Pulmonary Kinematics from Image Data: A Review}'' \emph{Acad
Radiol} 18, no. 4 (2011): 402--17.
doi:\href{https://doi.org/10.1016/j.acra.2010.10.019}{10.1016/j.acra.2010.10.019}

\hypertarget{ref-Tustison:2011ad}{}
19. Tustison, N. J., Awate, S. P., Song, G., Cook, T. S., and Gee, J. C.
``\textbf{Point Set Registration Using Havrda-Charvat-Tsallis Entropy
Measures}'' \emph{IEEE Trans Med Imaging} 30, no. 2 (2011): 451--60.
doi:\href{https://doi.org/10.1109/TMI.2010.2086065}{10.1109/TMI.2010.2086065}

\hypertarget{ref-Tustison:2012aa}{}
20. Tustison, N. J., Song, G., Gee, James C, and Avants, B. B.
...
21. Tustison, N. J., Contrella, B., Altes, T. A., Avants, B. B., Lange,
...
22. Barbosa, E. M., Jr, Song, G., Tustison, N., Kreider, M., Gee, J. C.,
...
\hypertarget{ref-Qing:2014aa}{}
23. Qing, K., Altes, T. A., Tustison, N. J., Feng, X., Chen, X., Mata,
J. F., Miller, G. W., Lange, E. E. de, Tobias, W. A., Cates, G. D., Jr,
Brookeman, J. R., and Mugler, J. P., 3rd. ``\textbf{Rapid Acquisition of
Helium-3 and Proton Three-Dimensional Image Sets of the Human Lung in a
Single Breath-Hold Using Compressed Sensing}'' \emph{Magn Reson Med}
(2014): doi:\href{https://doi.org/10.1002/mrm.25499}{10.1002/mrm.25499}

\hypertarget{ref-Qing:2014ab}{}
24. Qing, K., Mugler, J. P., 3rd, Altes, T. A., Jiang, Y., Mata, J. F.,
...
25. Tustison, N. J., Qing, K., Wang, C., Altes, T. A., and Mugler, J.
...
26. Song, G., Tustison, N., and Gee, J. C. ``\textbf{Airway Tree
...
27. Tustison, N. J., Avants, B. B., Flors, L., Altes, T. A., Lange, E.
...
28. Teague, W. G., Tustison, N. J., and Altes, T. A.
...
\hypertarget{ref-Tustison:2010ab}{}
29. Tustison, N. J., Altes, T. A., Song, G., Lange, E. E. de, Mugler, J.
...
30. Song, G., Mortani Barbosa, E., Jr, Tustison, N., Gefter, W. B.,
...
31. Bajcsy, R. and Broit, C. ``\textbf{Matching of Deformed Images}''
\emph{Sixth international conferenceon pattern recognition (iCPR'82)}
(1982): 351--353.

\hypertarget{ref-Bajcsy:1989aa}{}
32. Bajcsy, R. and Kovacic, S. ``\textbf{Multiresolution Elastic
...
33. Gee, J. C., Reivich, M., and Bajcsy, R. ``\textbf{Elastically
...
34. Klein, A., Andersson, J., Ardekani, B. A., Ashburner, J., Avants,
...
35. Murphy, K., Ginneken, B. van, Reinhardt, J. M., Kabus, S., Ding, K.,
...
36. Tustison, N. J., Yang, Y., and Salerno, M. ``\textbf{Advanced
...
38. Avants, B. B., Yushkevich, P., Pluta, J., Minkoff, D., Korczykowski,
...
39. Tustison, N. J., Shrinidhi, K. L., Wintermark, M., Durst, C. R.,
...
40. Avants, B. B., Duda, J. T., Kilroy, E., Krasileva, K., Jann, K.,
...
41. Datta, R., Lee, J., Duda, J., Avants, B. B., Vite, C. H., Tseng, B.,
...
42. McMillan, C. T., Avants, B. B., Cook, P., Ungar, L., Trojanowski, J.
...
43. Cook, P. A., McMillan, C. T., Avants, B. B., Peelle, J. E., Gee, J.
...
44. Tustison, N. J., Avants, B. B., Cook, P. A., Kim, J., Whyte, J.,
...
45. Vannier, M. W., Butterfield, R. L., Jordan, D., Murphy, W. A.,
...
46. Dempster, A., Laird, N., and Rubin, D. ``\textbf{Maximum Likelihood
...
47. Cline, H. E., Lorensen, W. E., Kikinis, R., and Jolesz, F.
...
48. Kikinis, R., Shenton, M. E., Gerig, G., Martin, J., Anderson, M.,
...
49. Geman, S. and Geman, D. ``\textbf{Stochastic Relaxation, Gibbs
...
50. Held, K., Rota Kops, E., Krause, B. J., Wells, W. M., 3rd, Kikinis,
...
51. Van Leemput, K., Maes, F., Vandermeulen, D., and Suetens, P.
...
52. Ashburner, J. and Friston, K. J. ``\textbf{Unified Segmentation}''
...
53. Avants, B. B., Tustison, N. J., Wu, J., Cook, P. A., and Gee, J. C.
...
54. Altes, T., Johnson, M., III, J. M., Miller, G. W., Flors, L., Mata,
...
55. Kirby, M., Pike, D., Coxson, H. O., McCormack, D. G., and Parraga,
...
56. Sled, J. G., Zijdenbos, A. P., and Evans, A. C. ``\textbf{A
...
57. Boyes, R. G., Gunter, J. L., Frost, C., Janke, A. L., Yeatman, T.,
...
58. Wang, H. and Yushkevich, P. A. ``\textbf{Multi-Atlas Segmentation
...
59. Wang, H., Suh, J. W., Das, S. R., Pluta, J. B., Craige, C., and
...
60. Yushkevich, P. A., Wang, H., Pluta, J., Das, S. R., Craige, C.,
...
62. Manjón, J. V., Coupé, P., Martí-Bonmatí, L., Collins, D. L., and
...
63. Wang, H., Suh, J. W., Das, S. R., Pluta, J., Craige, C., and
Yushkevich, P. A. ``\textbf{Multi-Atlas Segmentation with Joint Label
Fusion}'' \emph{IEEE Trans Pattern Anal Mach Intell} (2012):
doi:\href{https://doi.org/10.1109/TPAMI.2012.143}{10.1109/TPAMI.2012.143}

\hypertarget{ref-Landman2012}{}
64. ``\textbf{MICCAI 2012 Workshop on Multi- Atlas Labeling}'' (2012):

\hypertarget{ref-Asman2013}{}
65. Asman, A., Akhondi-Asl, A., Wang, H., Tustison, N., Avants, B.,
...
66. Avants, B. B., Tustison, N. J., Song, G., Cook, P. A., Klein, A.,
...
67. Avants, B. B., Tustison, N. J., Stauffer, M., Song, G., Wu, B., and
...
68. Avants, B. B., Klein, A., Tustison, N. J., Woo, J., and Gee, J. C.
...
69. Das, S. R., Avants, B. B., Grossman, M., and Gee, J. C.
...
70. Sluimer, I., Schilham, A., Prokop, M., and Ginneken, B. van.
...
71. Tustison, N. J., Johnson, H. J., Rohlfing, T., Klein, A., Ghosh, S.
...
72. De Nunzio, G., Tommasi, E., Agrusti, A., Cataldo, R., De Mitri, I.,
...
73. Prasad, M. N., Brown, M. S., Ahmad, S., Abtin, F., Allen, J., Costa,
...
74. Wang, J., Li, F., and Li, Q. ``\textbf{Automated Segmentation of
Lungs with Severe Interstitial Lung Disease in CT}'' \emph{Med Phys} 36,
no. 10 (2009): 4592--9.

\hypertarget{ref-Rikxoort:2009aa}{}
75. Rikxoort, E. M. van, Hoop, B. de, Viergever, M. A., Prokop, M., and
...
76. Zheng, B., Leader, J. K., McMurray, J. M., Park, S. C., Fuhrman, C.
...
77. Nakamura, M., Wada, S., Miki, T., Shimada, Y., Suda, Y., and Tamura,
...
78. Lo, P., Ginneken, B. van, Reinhardt, J. M., and Bruijne, M. de.
...
79. Agam, G., Armato, S. G., 3rd, and Wu, C. ``\textbf{Vessel Tree
...
80. Korfiatis, P. D., Kalogeropoulou, C., Karahaliou, A. N., Kazantzi,
...
81. Qi, S., Triest, H. J. W. van, Yue, Y., Xu, M., and Kang, Y.
...
82. Doel, T., Gavaghan, D. J., and Grau, V. ``\textbf{Review of
...
83. Uppaluri, R., Hoffman, E. A., Sonka, M., Hartley, P. G.,
...
84. Rosas, I. O., Yao, J., Avila, N. A., Chow, C. K., Gahl, W. A., and
...
85. DeBoer, E. M., Swiercz, W., Heltshe, S. L., Anthony, M. M., Szefler,
...
86. Rikxoort, E. M. van and Ginneken, B. van. ``\textbf{Automated
...
87. Warfield, S. K., Zou, K. H., and Wells, W. M. ``\textbf{Simultaneous
...
88. Coxson, H. O., Rogers, R. M., Whittall, K. P., D'yachkova, Y., Paré,
...
89. Perez, A., 4th, Coxson, H. O., Hogg, J. C., Gibson, K., Thompson, P.
...
90. Coxson, H. O. and Rogers, R. M. ``\textbf{New Concepts in the
...
91. Stolk, J., Putter, H., Bakker, E. M., Shaker, S. B., Parr, D. G.,
...
92. Xu, Y., Sonka, M., McLennan, G., Guo, J., and Hoffman, E. A.
...
93. Gevenois, P. A., De Vuyst, P., Sy, M., Scillia, P., Chaminade, L.,
...
94. Hoffman, E. A., Simon, B. A., and McLennan, G. ``\textbf{State of
...
95. Aykac, D., Hoffman, E. A., McLennan, G., and Reinhardt, J. M.
...
96. Park, W., Hoffman, E. A., and Sonka, M. ``\textbf{Segmentation of
...
97. Ederle, J. R., Heussel, C. P., Hast, J., Fischer, B., Van Beek, E.
...
98. Tustison, N. J. and Gee, J. C. ``\textbf{Run-Length Matrices for
Texture Analysis}'' \emph{Insight Journal} (2008):

\hypertarget{ref-Tustison:2009aa}{}
99. Tustison, N. J. and Gee, J. C. ``\textbf{Stochastic Fractal
Dimension Image}'' \emph{Insight Journal} (2009):

\hypertarget{ref-Lo:2012aa}{}
100. Lo, P., Ginneken, B. van, Reinhardt, J. M., Yavarna, T., Jong, P.
A. de, Irving, B., Fetita, C., Ortner, M., Pinho, R., Sijbers, J.,
...
101. Feuerstein, M., Kitasaka, T., and Mori, K. ``\textbf{Adaptive
Branch Tracing and Image Sharpening for Airway Tree Extraction in 3-d
Chest CT}'' \emph{Proc. of second international workshop on pulmonary
image analysis} (2009):

\hypertarget{ref-Lee:2009aa}{}
102. Lee, J. and Reeves, A. P. ``\textbf{Segmentation of the Airway Tree
from Chest CT Using Local Volume of Interest}'' \emph{Proc. of second
international workshop on pulmonary image analysis} (2009):

\hypertarget{ref-Staring:2012aa}{}
103. Staring, M., Xiao, C., Shamonin, D. P., and Stoel, B. C.
...
104. Yushkevich, P. A., Piven, J., Hazlett, H. C., Smith, R. G., Ho, S.,
...
105. Caselles, V., Kimmel, R., and Sapiro, G. ``\textbf{Geodesic Active
Contours}'' \emph{Int J Comput Vision} 22, (1997): 61--79.

\hypertarget{ref-Zhu:1996aa}{}
106. Zhu, S. and Yuille, A. ``\textbf{Region Competition: Unifying
...
107. Available at \url{http://www.itksnap.org}

\hypertarget{ref-Dregely:2011aa}{}
108. Dregely, I., Mugler, J. P., 3rd, Ruset, I. C., Altes, T. A., Mata,
...
109. Mugler, J. P., 3rd, Altes, T. A., Ruset, I. C., Dregely, I. M.,
...
110. Altes, T. A., Mata, J., Lange, E. E. de, Brookeman, J. R., and
...
111. Woodhouse, N., Wild, J. M., Paley, M. N. J., Fichele, S., Said, Z.,
...
112. Altes, T. A., Powers, P. L., Knight-Scott, J., Rakes, G.,
